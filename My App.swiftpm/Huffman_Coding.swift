//
//  Huffman_Coding.swift
//  HuffEncoder
//
//  Created by Giulia Casucci on 29/03/23.
//

///Based on Al Stevens' C Programming column from Dr.Dobb's Magazine, February 1991 and October 1992.

import Foundation

public class Huffman{
    ///Tree nodes don't use pointers to refer to each other, but simple integer indices. That allows us to use structs for the nodes.
    typealias NodeIndex = Int

    ///A node in the compression tree. Leaf nodes represent the actual bytes that are present in the input data.
    ///The count of an intermediary node is the sum of the counts of all nodes below it.
    ///The root node's count is the number of bytes in the original, uncompressed input data.
    struct Node {
        var count = 0
    ...
    ///The tree structure. The first 256 entries are for the leaf nodes (not all of those may be used, depending on the input). We add additional nodes as we build the tree.
    var tree = [Node](repeating: Node(), count: 256)

    ///This is the last node we add to the tree.
    var root: NodeIndex = -1

    ///The frequency table describes how often a byte occurs in the input data. You need it to decompress the Huffman-encoded data. The frequency table should be serialized along with the compressed data.
    public struct Freq {
        var byte: UInt8 = 0
        var count = 0
    }
...
extension Huffman {
    ///To compress a block of data, first we need to count how often each byte occurs. These counts are stored in the first 256 nodes in the tree, i.e. the leaf nodes. The frequency table used by decompression is derived from this.
    ...
    ///Returns the frequency table. This is the first 256 nodes from the tree but only those that are actually used, without the parent/left/right pointers. You would serialize this along with the compressed file.
...
extension Huffman {
    ...
    ///Recursively walks the tree from a leaf node up to the root, and then back again. If a child is the right node, we emit a 0 bit; if it's the left node, we emit a 1 bit.
...
extension Huffman {
    ...
    ///Walks the tree from the root down to the leaf node. At every node, read the next bit and use that to determine whether to step to the left or right. When we get to the leaf node, we simply return its index, which is equal to the original byte value.
